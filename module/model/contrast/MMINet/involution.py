...
class Involution1D(nn.Module):
    """
    This class implements the 1d involution analogous to the 2d involution.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        :param sigma_mapping: (nn.Module) Non-linear mapping as introduced in the paper.
                              If none, BN + ReLU is utilized.
        :param kernel_size: (Union[int, Tuple[int]]) Kernel size to be used
        :param stride: (Union[int, Tuple[int]]) Stride factor to be utilized
        :param groups: (int) Number of groups to be employed
        :param reduce_ratio: (int) Reduce ratio of involution channels
        :param dilation: (Union[int, Tuple[int]]) Dilation in unfold to be employed
        :param padding: (Union[int, Tuple[int]]) Padding to be used in the unfolding operation
        ...
        # Save parameters (convert to 1-tuple if not already)
        ...
        self.sigma_mapping = (
            ...
            if sigma_mapping is not None
                ...
                nn.BatchNorm1d(num_features=self.out_channels // self.reduce_ratio, momentum=0.3),
                nn.ReLU(),
            )
        )
        ...
        self.o_mapping = nn.AvgPool1d(kernel_size=self.stride[0], stride=self.stride[0])
        self.reduce_mapping = nn.Conv1d(
            in_channels=self.in_channels,
        ...
        self.span_mapping = nn.Conv1d(
            in_channels=self.out_channels // self.reduce_ratio,
            out_channels=self.kernel_size[0] * self.groups,
            kernel_size=1,
            stride=1,
            padding=0,
            bias=bias,
        )
        # Create a 2d-unfold layer treating the 1d signal as a 2d image with height=1
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        """
        Forward pass
        :param input: (torch.Tensor) Input tensor of the shape [batch size, in channels, width]
        :return: (torch.Tensor) Output tensor of the shape [batch size, out channels, width] (w/ same padding)
        """
        # Check input dimension (should be 3D: [B, C, width])
        assert (
            input.ndimension() == 3
        ), "Input tensor to involution must be 3d but {}d tensor is given".format(
            input.ndimension()
        )
        # Save input shape and compute output width
        batch_size, _, in_width = input.shape
        out_width = (
            in_width + 2 * self.padding[0] - self.dilation[0] * (self.kernel_size[0] - 1) - 1
        ) // self.stride[0] + 1
        # Apply the initial mapping and unsqueeze to add a dummy height dimension for unfolding
        x = self.initial_mapping(input).unsqueeze(2)  # shape: [B, out_channels, 1, width]
        # Unfold and reshape input tensor
        input_unfolded = self.unfold(x)
        # The resulting shape is [B, out_channels*(1*self.kernel_size[0]), out_width]
        # Reshape to [B, groups, out_channels//groups, kernel_size, out_width]
        ...
        # Optionally, adjust input shape to avoid mismatches
        if self.force_shape_match:
            input = F.adaptive_avg_pool1d(input, out_width)
        # Generate kernel
        kernel = self.span_mapping(self.sigma_mapping(self.reduce_mapping(self.o_mapping(input))))
        # kernel shape: [B, kernel_size[0]*groups, out_width]
        kernel = kernel.view(
            batch_size, self.groups, self.kernel_size[0], kernel.shape[-1]
        ).unsqueeze(
            dim=2
        )  # shape: [B, groups, 1, kernel_size, out_width]
        # Apply kernel to produce output
        output = (kernel * input_unfolded).sum(dim=3)
        # Reshape output to [B, out_channels, out_width]
        output = output.view(batch_size, -1, out_width)
        return output
...
class Involution2d(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.unfold = nn.Unfold(
            kernel_size=self.kernel_size, dilation=dilation, padding=padding, stride=stride
        )
...
class Involution3d(nn.Module):
        ...
        **kwargs
        ...
        self.sigma_mapping = (
            ...
            if sigma_mapping is not None
                ...
                nn.BatchNorm3d(num_features=self.out_channels // self.reduce_ratio, momentum=0.3),
                nn.ReLU(),
            )
        )
        self.initial_mapping = (
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        assert (
            input.ndimension() == 5
        ), "Input tensor to involution must be 5d but {}d tensor is given".format(
            input.ndimension()
        )
        ...
        input_unfolded = input_unfolded.reshape(
            batch_size,
            self.groups,
            self.out_channels // self.groups,
        ...
        input_unfolded = input_unfolded.reshape(
            tuple(input_unfolded.shape[:-1]) + (out_depth, out_height, out_width)
        )
        # Reshape input to avoid shape mismatch problems
        if self.force_shape_match:
